...
We are going to follow an Open-Ended Evolution (OEE) approach.  OEE is a type of genetic evolution algorithm in which the goal is
...
The organization by Renz et al. \cite{Renz, Renz2015TheAB} of an AI
competition to develop intelligent agents that could play new Angry
Birds levels better than the best human players was a very interesting
boost to this area, and also boosted in general PCG by offering a
challenge in different levels: from the physical, structural level, to
the game play and aesthetic one.

To better train and test agents, more
novel and challenging levels must be generated. Stephenson et
...
layout of the Levels. This interactive approach could, in principle, be also approached in the generation of Angry Birds levels.

In our case, interaction is not required or used; our main intention
is to generate interesting and aesthetically pleasing levels. The
methodology to achieve this is explained below. 
...
\item   The number of \textit{different} levels that must be generated. 
...
We will present next a system for evolving structures that respects
those restrictions.
...
composite in the pool. A chromosome can include the same ID multiple times.
    ...
    of pieces found before the simulation started. Each block represents a certain percentage so the missing pieces are subtracted from the total. In this
    moment, every block is counted as if it was not a member of a
    composite since after the simulation there is only data about each individual block. 
    ...
    % is shown in Figure \ref{new_model} in xwhich the parameter file of the competition 
...
The system includes all these components:
    ...
    \begin{figure}[htbp]
        ...
        \caption{Combination of the composites layer and the mask layer.}
        \label{layer12_combine}
    \end{figure}
    
    \begin{figure}[htbp]
        \centerline{\includegraphics[width=80mm]{Images/layer123_combine.png}}
        \caption{Combination of the layers in the previous figure with the pig layer.}
...
continue running the experiment for more generations, since their fitness value could drop due to similarity with other levels.  
...
In general, in this work we were able to fulfill our objective of
    generating levels that could be played, and that integrated
    generation of interesting structures together with the placement
    of pigs using a {\em layer} system that allowed the simultaneous
    evolution of both. Evolution took into account both intrinsic
    characteristics of the structure as well as diversity, allowing
    the continuous production of novel structures, without having to
    wait for the last generation. As a matter of fact and looking at
    the competition for which this system was originally created, we
    would produce the first 100 structurally sound levels as soon as
    they go beyond a certain fitness threshold.

Designing a fitness function is one of the most challenging parts of
evolving levels; all the main objectives of the evolutionary process
have to be taken into account, which is why we use the number of
remaining building blocks after simulation, together with the general
structure of the level, and another component that takes into account
how different this structure is from the rest. Besides allowing single
processes to generate diverse structures, this also helps the
evolutionary process search into new parts of the structure space,
helping the objective of generating many structures, with a high
fitness, at the same time.

    
The figures and resulting levels show, however, that there is room for
improvement, and some modifications must be applied in order to
optimize some aspects of it. For instance by using a different way of
generating individual we were able to reduce the process time required
on a full system execution; however we still need to work on this
problem as we mention before.
